...
export default function CartProvider({ children }: PropsWithChildren) {
  ...
  useEffect(() => {
    const loadCart = async () => {
      try {
        ...
        const response = await axios.get(`https://lemontoys-server.vercel.app/cart?userId=${userId}`);
  ...
  const AddToCart = async (item: any) => {
    try {
      ...
      if (existingItemIndex !== -1) {
        ...
        // Sync with backend
        await axios.post(`https://lemontoys-server.vercel.app/add-to-cart`, {
          userId,
          product: {
            ...item,
        ...
        // Create in backend
        const response = await axios.post(`https://lemontoys-server.vercel.app/add-to-cart`, {
          userId,
          product: {
            ...item,
  ...
  const RemoveFromCart = async (item: any) => {
    try {
      ...
      await axios.post(`https://lemontoys-server.vercel.app/remove-from-cart`, {
        userId,
        productId: item._id,
  ...
  const deleteFromCart = async (item: any) => {
    try {
      ...
      await axios.post(`https://lemontoys-server.vercel.app/delete-from-cart`, {
        userId,
        productId: item._id,
      });
    } catch (error) {
  ...
  const clearCart = async () => {
    try {
      ...
      // Sync with backend
      await axios.post(`https://lemontoys-server.vercel.app/clear-cart`, {
        userId,
      });
    } catch (error) {
